package com.vanancrm.TestCases.CRM;
...
public class ReposonseAndUnresonseCount extends TestBase {
    ...
    public void testStep() throws InterruptedException, IOException {
        ...
        for (int i = 0; i < menu.size(); i++) {
            ...
            if (i <= size) {
                ...
                selectMenu(menu.get(i));
                checkOrderListScenario(respond.get(i), unrespond.get(i), false);
            }
        }
    ...
    private void checkScenarioForRequestRefund(String respond, String unrespond)
    ...
    private void checkOrderListScenario(String respond, String unrespond,
            boolean refund) throws IOException, InterruptedException {

        checkCountOrderList("Reposond count is ", respond, true, refund);

        if (!unrespond.equals("#")) {
            clickTicket(unrespond);
            checkCountOrderList("Unreposond count is ", unrespond, false, refund);
        }
        System.out.println("\n");
    }
...
*/
        ...
        try {
            ...
            if (element.isDisplayed()) {
                ...
                if (value == 0) {
                    ...
                    if (value >= 50) {
                        if (value == getLastTicketNumberFromTAT()) {
    ...
    private void checkCountOrderList(String message, String element, boolean
            respondStatus, boolean refund) throws IOException,
            InterruptedException {
        if (isRespondDisplayed(element)) {
            System.out.print(message + " : " + getCount(element));
            checkStatusOrderList(element, true, refund);
        } else {
            System.out.println(message + " zero [" + getCount(element)
                    + "]");
            if (respondStatus) {
    ...
    private void checkStatus(String element, boolean respondStatus) throws
        ...
        menus.waitForPageLoad(driver);
        int acount = getLastTicketNumber();
        if (ecount >= 50) {
            if (ecount == acount) {
    ...
    private void checkStatusOrderList(String element, boolean respondStatus,
        ...
        int acount = 0;
        if(refund) {
            acount = getLastTicketNumberForRefundRequest();
        } else {
            acount = getLastTicketNumberFromTAT();
        }
    ...
    private int getTableRowSize() {
        ...
        List<WebElement> elements = driver.findElements(By.xpath
                ("//div[@id='process_lists_wrapper']/table/tbody/tr"));
